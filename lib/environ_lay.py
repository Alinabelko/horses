...
MINIMAL_BET = 4.0
KEEP_REWARD = 0.005
OPEN_BET_PENALTY = 0.005
CLOSE_BET_PENALTY = 0.00
ZERO_BET_PENALTY = 0.0

SHORT_BET_MAX_PENALTY = 0.5
SHORT_BET_KEEP_LENGTH = 10
...
class State:
    ...
    def step(self, action):
        ...
        if action == Actions.Close and self.have_position:
            close_price = self._prices.back_price[self._offset]
            close_size = self.lay_value / close_price
            close_size = - close_size
            #add double reward on close
            #reward = (1 - self.open_price / close_price) * 2       
            reward = 1 - self.open_price / close_price            
            if(reward == 0):
              reward -= ZERO_BET_PENALTY
            self.print_log(reward, close_price)
            if(self.keep_duration < SHORT_BET_KEEP_LENGTH):
                penalty = (SHORT_BET_MAX_PENALTY/SHORT_BET_KEEP_LENGTH) * (SHORT_BET_KEEP_LENGTH - self.keep_duration)
                reward -= penalty
        ...
        if self.have_position:
          ...
          if(self.bet_type == Actions.Lay):
            close = self._prices.back_price[self._offset] 
...
class StocksEnvLay(gym.Env):
    ...
    def from_dir(cls, data_dir, **kwargs):
        ...
        return StocksEnvLay(prices, **kwargs)